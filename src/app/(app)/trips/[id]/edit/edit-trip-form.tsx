...
import { updateTripAction, type TripEditState } from '../../actions';
import { locationTypes, weatherConditions, type Trip } from '@/db/schema';
...
export default function EditTripForm({ trip }: { trip: Trip }) {
  ...
  const [state, action, pending] = useActionState<TripEditState | undefined, FormData>(
    boundAction,
    undefined
  );
    ...
    return (
      <div className="text-center space-y-6 py-4">
        <div className="space-y-2">
          ...
          <h2 className="text-xl font-black uppercase tracking-wide text-foreground">Trip Updated</h2>
          <p className="text-sm text-muted-foreground">The driving session has been saved.</p>
        </div>
        <Link
          href="/trips"
          className="inline-block rounded bg-primary px-6 py-2.5 text-sm font-bold text-primary-foreground uppercase tracking-widest hover:opacity-90"
        >
          Back to Trips
        </Link>
      </div>
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div>
        ...
        <select id="locationType" name="locationType" required defaultValue={trip.locationType} className={inputClass}>
      ...
      <div>
        ...
        <select id="weather" name="weather" required defaultValue={trip.weather} className={inputClass}>
          <option value="">Select…</option>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            min="0"
            max="600"
            defaultValue={trip.daytimeMinutes}
            className={inputClass}
          />
        </div>
        <div>
          ...
          <input
            ...
            min="0"
            max="600"
            defaultValue={trip.nighttimeMinutes}
            className={inputClass}
          />
        </div>
      </div>
